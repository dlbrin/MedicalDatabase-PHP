...
<html lang="en">
	...
	<body>
		...
		if(isset($_GET['search'])){
			$data = ($_GET['search']);
			$sql = mysqli_query($db , "SELECT * FROM `sick` WHERE `answer` = 'Replay' AND `name` LIKE '%$data%' OR `surgery_date` LIKE '%$data%' OR `number` LIKE '%$data%' OR `types_of_transplant` LIKE '%$data%' ");
		...
		<table class="table-naxosh" width="100%">
			...
			<?php
				$query = mysqli_query($db , "SELECT * FROM `sick` WHERE `answer` = 'Pending' ORDER BY `surgery_date` DESC");
